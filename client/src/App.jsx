...
function AdvancedPanel({ user, onClose, onSaved }) {
  ...
  const [room, setRoom] = useState(() => localStorage.getItem('skillswap-video-room') || '');
  const [status, setStatus] = useState('');
  ...
  const createRoom = async () => {
    ...
    try {
      ...
      setRoom(result.url);
      localStorage.setItem('skillswap-video-room', result.url);
      setStatus('Video room created. Share the meeting link with your exchange partner.');
    } catch (error) {

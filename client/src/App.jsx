...
function HowItWorksPage({ currentUser, skills, onClose, onConnect }) {
  ...
  const demoSkills = skills.length ? skills : [{ title: 'Photography', teacher: { name: 'SkillSwap member', role: 'Community member', avatar: 'SS' }, wants: 'A useful skill exchange', format: 'Flexible' }];
  const [demoSkill, setDemoSkill] = useState(demoSkills[0]?.title || 'Photography');
  const [openFaq, setOpenFaq] = useState(0);

  useEffect(() => {
    if (demoSkills.length && !demoSkills.some((skill) => skill.title === demoSkill)) {
      setDemoSkill(demoSkills[0].title);
    }
  }, [skills, demoSkill]);
  ...
  const selectedSkill = demoSkills.find((skill) => skill.title === demoSkill) || demoSkills[0];

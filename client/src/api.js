...
export async function updatePortfolio(id, payload) { const response = await fetchWithAuth(`${API_URL}/profiles/${encodeURIComponent(id)}/portfolio`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) }); return storeSessionToken(await readResponse(response, 'Could not save portfolio')); }
